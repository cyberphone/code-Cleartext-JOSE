...
public class Signatures {
    ...
    static String cleanJavaScriptSignature(byte[] signature) throws IOException {
        ...
        int i = text.indexOf(" " + JSONCryptoHelper.SIGNATURE_JSON + ": \"");
        int j = text.indexOf('"', i + JSONCryptoHelper.SIGNATURE_JSON.length() + 4);
    ...
    static String cleanSignature(byte[] signedData) throws IOException {
        ...
        JSONObjectReader signature = reader.getObject(JSONCryptoHelper.SIGNATURE_LABEL_JSON);
        if (signature.hasProperty(JSONCryptoHelper.SIGNERS_JSON)) {
            ...
            while (array.hasMore()) {
                array.getObject().removeProperty(JSONCryptoHelper.SIGNATURE_JSON);
            }
        } else {
            signature.removeProperty(JSONCryptoHelper.SIGNATURE_JSON);
        }
    ...
    static byte[] createSignature(JSONSigner signer) throws Exception {
        ...
        int i = signed.indexOf(",\n  \"" + JSONCryptoHelper.SIGNATURE_LABEL_JSON + "\":");
